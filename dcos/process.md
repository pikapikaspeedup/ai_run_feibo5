...
## 2026-07-08 - XP 磁吸修复（"绿色点点无法吸"）

- 用户反馈：绿色点点无法吸，不知道是什么。判定：就是 XP 经验豆——拾取判定 52px 内 170px/s 缓慢漂移（等于要踩上去），v2.9 击杀量翻倍且多为远程击杀后，经验豆满地躺尸。
- 修复：进 110px 圈标记起飞，130→560px/s 加速追踪玩家直到入袋（VS 磁吸手感）；满血时血包不追（留给残血时吃，带护盾的饼例外照吃）。
- Verification: preview——100px 处 XP 0.25s 起飞入袋✅；300px 处 60 帧不动（保留探索/走位价值）✅；残血后血包即刻入袋✅；build 通过零 error。（"满血血包不动"用例在实战环境被 bot 蹭血干扰无法隔离复现，谓词逻辑为单行 hp<maxHp 判断，行为由残血用例覆盖。）

## 2026-07-08 - v3.0 前期体验 VS 化（第一批）

- 用户拍板：围绕吸血鬼幸存者做前期体验。本批交付：
  ① 经验落地化（结构改动）：玩家杀怪经验不再无声进账，落地成豆（磁吸捡取）；同点合豆（24px 并入、全场 220 上限溢出并最近豆）；豆分级渲染（≥8 大豆、≥25 发光大豆）。同事/精英击杀与 bot 击杀仍直接进账。
  ② 升职体检=满血复活（原 +8HP）——残血搏升级的"又容易又不容易"张力。
  ③ 杂鱼 5% 掉咖啡豆（hpBase≥8 才掉，防 A4 纸人刷屏）；咖啡豆渲染改绿十字 + 满血踩过节流提示"存着救命"——绿点之谜终结。
  ④ 人墙波「需求洪峰」：BR 阶段每 40-60s 一面 30+10×phase 只低血杂鱼墙 waveRush 直线冲锋（终圈 phase≥4 停发），总量 120 封顶；涓流 9s→7s/10-16 只/基线 60。
  ⑤ 试用期波次 ×1.25（月1）/×1.4（月2+）——生存工具箱变厚后把 v2.1 砍掉的密度还回来。
  ⑥ 新道具「扫地机器人」：全场经验豆起飞代收（残血连咖啡豆），VS 磁铁时刻。
  ⑦ 被打不再无脑震屏：单发≥12% 最大生命才震（50 人局挨打是常态，原来每中一发抖一下=用户报告"被打时位置在漂"）。
- Verification: 升级满血✅ 杀怪掉豆且不直接进账✅ 同点 5 杀并成 1 颗 26.6 豆✅ 人墙波 13→38 只/25 rush 标记+截图确认横排压境✅ 扫地机器人 6 豆全起飞入袋 +40.6xp✅ 小伤不震/25% 重击震 2✅ 月1波1 target 16→20✅ 109 只怪 update 0.59ms（预算 16.7ms）✅ build 通过、控制台零 error。
- 遗留（VS 前期还差的，已诊断未做）：杂鱼 TTK 按武器整刀化（DeepSeek 5 发/邮件太肉）、波形花样（圆环合围/十字夹击）、精英=宝箱开箱演出、分钟级压力脚本、顿帧/大数字打击感包（B 演出线）、连杀播音员。

## 2026-07-08 - v3.0.1 人口控制器（"怪还是太少"的根治）

- 用户反馈：怪还是太少，要有合理的设计。反思：前两轮都在调"刷怪参数"（间隔/每波只数/上限），
  但 VS 的本质是人口控制器——每阶段一个"场上最低怪数"，杀多快补多快；波次只是地板之上的尖峰。
- 设计（shuang-design-v2.9.md 三·b）：floor 曲线 试用期 16+7m（月1=23→月5=51）/ 正赛 phase0-2 =50/65/80、
  phase3=45、终圈=30，硬上限 150；每 0.7s 补 ≤10 只，出生环 380-460px（视野半对角≈367 刚好屏外）；
  后台怪经验 7 折防等级通胀；波次怪计入人口（倾倒期控制器歇火，波间空窗被填满，kill-target 结构不破坏——
  ambient 怪无 trialSubWave 标记，进度零污染）。
- 回收器：每 2s 把 750px 外普通怪传送回出生环（人口守恒不删怪），锚点怪（据点/蛇/编队/站桩/精英）除外——
  顺带根治"怪卡远处死角拖波次"整类问题。原 BR"7 秒涓流包"删除；人墙波保留为事件尖峰（封顶 150）。
- Verification: 试用期月1 60s 采样 avg 27.6 / min 20 / max 48（波间不再空场）✅ 杀 3 只后台怪波次计数不动✅
  60s 正常推进到第 3 波✅；BR phase0 45s 采样 avg 46.9（地板 50，min 10 为开局爬坡）✅；
  900px 外怪 2.5s 传送回 353px✅；人墙波瞬时冲到 78 只✅；148 只怪 update 1.35ms（预算 16.7ms）✅；
  build 通过、控制台零 error。
- 环境坑注：预览标签页 document.hidden=true 时 rAF 完全挂起（实时观测截图会冻结在 00:00）——
  引擎验证一律手动驱动 core.update()，实况截图仅在标签可见的会话里拍。

## 2026-07-10 - v3.1 武器扩容：玄学科+风暴科 12 武器 + 6 传说 + codex 素材 8 组

- 设计（dcos/weapons-v3.1-design.md）：VS 武器多样性=几何独占。补齐未占几何：贴身光环/随机点名空袭/弹墙台球/计数爆发/尾迹雷/随机水洼/命中弹射/命中分裂/火墙/物理锤/剑气/引力井。
- SP 玄学科：电子木鱼(karma 功德计数→冲击环)、驳回印章(stamp 随机空袭+红印decal)、踢皮球(bounce 弹墙加速)、隔夜韭菜盒子(复活 aura 死代码+Lv5 fear)、历史遗留问题(复活 mine 死代码+Lv5 连环塌方)、空调漏水(rain 随机水洼)。
- ST 风暴科：群发转发邮件(ricochet 每跳+15%伤)、需求拆解(split 命中裂 3+孙需求)、火线需求(firewall burns 排墙)、领导拍板(slam 击飞+撞人传伤 shoveWithImpact)、裁员大刀(slashwave 贯穿月牙)、强制拉会(vortex 引力井→burns.pull)。
- 传说 6：电子功德林(功德环+章雨)、舆情失控(每跳分裂 capped 260 projs)、毕业典礼(拍板+四向刀气)、项目火葬场(漩涡+合拢火圈)、击鼓传锅(弹锅沿途埋雷)、茶水间生化事故(光环+毒滩蔓延)。RECIPES 8→14 对。
- 引擎：spawnBullet 新字段 ricochet/split/wallBounce/vortexOn/potMine/ignoreUnit/_viralSplit；proj 命中钩子(弹射重定向/分裂生成/漩涡触发)；世界边缘+掩体反弹 bounceProj(按穿透深度选反射轴)；burns 新能力 pull 引力/mineChain 链爆；helpers stampStrike/shoveWithImpact/triggerVortex。
- 素材：codex CLI 0.133→0.144 升级后 8 条航道全过（fx_karma/stamp/vortex/slam/firewall/stink/acdrip 3x3 动画 + props 表 9 件）；切片 72 帧 fx + 6 proj 单帧 + decal_f18 驳回印 + 2 备用 prop；ONESHOT_FX 表接 5 新 fx；drawProj 新 6 弹形程序化兜底(wave/plane/docstack/ball/pot/invite)；恶臭光环波纹圈可视化。
- 踩坑：①codex 旧版对 gpt-5.6-sol 报 400，upgrade 后重跑航道即可；②Edit 助手函数时误删 function updateWeapon 函数头导致 esbuild Unexpected "}"——用 esbuild 单文件秒定位；③击鼓传锅首测 0 杀是测试口径（30hp 怪需两锅+每帧拽回原位踩不到雷），机制探针（5 锅/45 雷/静止靶-52hp）证明正常。
- Verification: 12/12 武器 + 6/6 传说逐一引擎击杀测试通过（表见上轮 eval 输出）；build 通过；控制台零 error。UI：图鉴新增精神防御科/职场风暴科分区，Hud 国别标签"玄学/风暴"。

## 2026-07-10 - v3.2 素材强化 + 边框残留全库清理（用户报键盘刀气方框）

- 新增素材（codex 航道 C+D，8 次生成）：fx 动画 3 组接线（orbit_chair 转椅 9 帧自旋/orbit_blade 键盘刀 9 帧/proj_kickball 皮球滚动 9 帧——环绕武器与皮球从程序化方块升级为实体贴图）；武器图标 4 张表 → chip_<id>.png 17 件新图标（16 新武器 + item_robot_vac 扫地机器人），地面芯片/图鉴 DexIcon/升级抽卡武器卡三处 UI 全部接图标（icons.js 新增 WPN_ICONS 装载器）。
- 接线补欠账：fx_firewall 循环帧接 fireWall burns（燃烧便签墙实体化）、fx_stink 接恶臭光环叠加、prop_todo_pit 接地雷贴图（未武装半透明）、prop_wooden_fish 接木鱼悬浮（敲击节奏微缩放脉动）、fx_slash 老素材接进 v2.9 slash 分支（月牙帧优先/全圆弧程序化）。
- 【重大踩坑·必记】magick -crop 341x341 在 1024 图上产出 4×4=16 块（341×3=1023 余 1px 切成细条）：cell_3/cell_7 是 1px 绿条，真实九宫格内容在 tile (k/3)*4+k%3 = 0,1,2,4,5,6,8,9,10。v3.1+v3.2 首轮切片全部错位（印章 decal 装了铁锅、木鱼图标是空条等）——全量按正确索引重切修复。以后切 3x3 一律用显式 tile 映射或先 resize 1023x1023。
- 【第二坑】remove_chroma_key 对 1px 细条格全判透明（引发"两张表都死 3/7 号位"的假线索——实为切片错位的下游症状）。
- 边框残留清理：生成图九宫格自带淡格线，切片边界留半透明线框，128px 贴图放大画=明显方框（用户截图键盘刀气+红怪均中招）。写批量清理器：外圈 2px alpha<240 清透明 + 外侧 4px 行列"成线判定"（占比>55% 且比内侧高 25pp）整行清除。全库扫 1458 张、清理 161 张、复扫余 0；重切文件二次过清理器。
- 调参：vortexfx 显示倍率 2.4→1.6（300px 紫风车盖半屏太抢戏）。
- Verification: 重切后 127 目标文件仅 3 张"近空"均为设计淡出帧；chip_wooden_fish/keyboard/ac_leak/boss_slam 目检正确（键盘砍刀/金木鱼/滴水空调/金巨掌）；decal_f18 红印、proj_pot_relay 铁锅归位；游戏内实拍（手动 render 绕过后台标签 rAF 挂起）：漩涡/火墙/皮球全部无边框，bot 持新武器的击杀播报自然成立（"用「裁员大刀」下架了…"）；build 通过；控制台零 error。

## 2026-07-10 - 漩涡旋转修复（用户报"这不是应该要能旋转吗"）

- 原因：vortexfx 走 ONESHOT 通道——9 帧铺在 1.2s 生命上单次顺播（≈7.5fps），且不做旋转变换；生成帧虽有相位差（f0 vs f2/4/6 平均像素差 111-191 实测），但 6 臂对称风车转 40° 视觉上≈抖动，整体读作静止贴图。
- 修复：vortexfx 独立 draw 分支双保险——ctx.rotate(f.t×5.5) 程序化旋转 + 帧序循环播放（(f.t×2.2)%1），不再赌生成帧相位；素材缺失时三臂渐隐螺旋程序化兜底；alpha 封顶 .92。
- Verification: 0.25s 间隔两次手动渲染，漩涡中心 80×80 像素哈希不同（3768951819→3212396295）✅；实拍尺寸 1.6 倍率下不再盖半屏；控制台零 error；build 通过。

## 2026-07-10 - 水滩/火墙素材返工（用户报"一个没有好素材，一个素材很糟糕"）

- 水滩（空调漏水/饮水机/会议水渍/蜗牛拖尾/灭火器泡沫）：一直是程序化 18% 透明圆片，多个叠加像色块——新生成 fx_waterpool 九帧波光循环（不规则蓝水泊+白涟漪，magenta 键），burns 管线原生 spr 字段直接挂上（8 处 G.burns.push 打标）。
- 火墙（火线需求/项目火葬场）：原 fx_firewall 素材是"三张便签一排"整图，每段墙重复画整排 = 复制粘贴火箱子——重新生成 fx_firenote 单张燃烧便签（便签小火苗大、带翘角），每段一张火苗朝上排成连续火线；fx_firewall 九帧删除。
- Verification: 同屏摆拍 14 段火墙 + 24 个水滩实拍确认（水泊波光/单便签火线成型）；切片走正确 16-tile 映射+边框清理零残留；build 通过；控制台零 error。

## 2026-07-10 - 引力井去圆盘（用户报"紫色背景有点多余"）

- 原因：引力井由两件套组成——vortexfx 风车特效 + 承载拉力的 burns 力场；无贴图 burns 默认画 18% 透明色圆兜底，pullR 90-130 的力场就成了盖在风车下面的紫色大圆盘。
- 修复：burns 加 noDraw 字段（纯逻辑力场不渲染），引力井力场打标；范围表达全权交给风车本体。
- Verification: 漩涡生命中段定格实拍——风车在转、圆盘消失；pullFieldNoDraw=true；build 通过；控制台零 error。

## 2026-07-11 - 弹射椅卡波修复（用户报"两个怪疯狂飞追不上，波次完不成，一直升级"）

- 定性：三层防护网全部漏掉同一只怪。那两只=chair_crazy 人体工学椅暴走（月2波1 恰好 ×2）。
  ①bounce 弹射分支在 waveRush 检查之前就 return——v2.6 清场冲锋只打通了 keepDist/thief 两分支，
  椅子 240px/s 满图乱弹无视清场令；②v3.0 回收器用 spdBase<=0 豁免站桩怪，chair_crazy 恰好 spd:0
  （弹射自驱）→ 弹出 750px 永不归位；③审计器只治计数漂移不治"活着但在天边"；
  ④v3.0 人口控制器让卡波变成无限刷级农场（后台怪不断→一直升级），放大了症状可见度。
- 修复四层：①bounce 分支 waveRush 特化——不休息+每帧锁玩家方向（保留弹射演出但必冲人）；
  ②回收器豁免改显式静态类型集 STATIC_MOBS（night_lamp/deadline_alarm/req_review_board/smoker/urgent_meeting），
  回收时重掷 bounceDir（否则回来一帧又朝原方向弹走）；③保持原对账；
  ④新增终极兜底：waveStuckT≥40s 残余波怪由"HR 代为处理"强制归档（飘字+粒子+feed 梗化），
  设计上保证波次永远不可能无限卡死。
- Verification: 完整复现——月2波1 留 3 只椅子扔 1200px 外：回收拉回✅→waveRush 冲锋✅→波次推进✅（未触发兜底，自然解决）；
  单测兜底——把波怪每帧钉在 2000px 外+stuckT 拨 38：HR 强杀✅ 波次推进✅；build 通过；控制台零 error。

## 2026-07-11 - v3.3 界面治理 + 经济重调（用户报"界面太乱/升级太快/平衡差"）

- 三根因：①v2.5 电子木鱼技能功德无上限——割草量×20 后实测银行 2089 功德=69 条免死（"平衡差"元凶），
  且"每 5 杀飘一次"在新杀速下刷成柱压住 HUD；②空调漏水水滩半径吃 dropR×等级×mods.range 三乘区，
  250px 巨湖数滩相叠半屏全蓝；③升级节奏未随 v3.0 密度×3 重调（白板全托管实测 2min Lv8 / 5min Lv13，
  真实玩家滚雪球更爆）。
- 视觉预算：飘字全局 120→26；功德飘字 10 杀一报+2s CD+敲击音降频；咖啡豆散豆≤12+30px 并豆；
  水滩生成半径封顶 64 + 贴图地面区绘制封顶 70（判定不变）。
- 平衡：功德银行封顶 60（最多 2 条命）；levelNeed 1.22→1.27；后台怪经验 0.7→0.5。
- Verification: 白板 sim 基线 t60/120/180/240/300 = 4/8/10/11/13 → 调后 4/5/6/8/9-10（t60 前期爽感保留，
  转正白板 9-10=真实玩家 12-14 回到 Lv11-13 设计锚点）；飘字预算 cap 生效；build 通过；控制台零 error。
- 原则沉淀至 shuang-design-v2.9.md §七：视觉预算 + 经济锚点 + "击杀攒资源类必须封顶"。
